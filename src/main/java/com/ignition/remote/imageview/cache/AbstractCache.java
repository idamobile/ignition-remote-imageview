...
import com.ignition.remote.imageview.util.IgnitedStrings;
...
public abstract class AbstractCache<KeyT, ValT> implements Map<KeyT, ValT> {
    ...
    private Map<KeyT, Long> entriesTime;
    ...
    public AbstractCache(String name, int initialCapacity, long expirationInMinutes,
        ...
        this.entriesTime = new ConcurrentHashMap<KeyT, Long>();

        this.cache = new LruCache<KeyT, ValT>(initialCapacity) {
            @Override
            protected int sizeOf(KeyT key, ValT value) {
                return getSizeOf(key, value);
            };
        };
    }

    protected int getSizeOf(KeyT key, ValT value) {
        return 1;
    }
    ...
    public ValT get(Object elementKey) {
        ...
        if (file.exists()) {
            ...
            cache.put(key, value);
            entriesTime.put(key, SystemClock.elapsedRealtime());
            return value;
        }
    ...
    private boolean expired(Object elementKey) {
